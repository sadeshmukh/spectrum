...
const DELAY_CONFIG = {
  SEARCH_DELAY: { min: 500, max: 1000 },
  PRODUCT_DELAY: { min: 100, max: 200 },
};
    ...
    for (const searchUrl of searchUrls) {
      try {
        ...
        for (const url of productUrls) {
          try {
            await delay(
              DELAY_CONFIG.PRODUCT_DELAY.min,
              DELAY_CONFIG.PRODUCT_DELAY.max
            );
        ...
        searchProgress.tick();
        await delay(
          DELAY_CONFIG.SEARCH_DELAY.min,
          DELAY_CONFIG.SEARCH_DELAY.max
        );
      } catch (searchError) {
